...
const views: ReadonlyArray<{ id: EpisodeViewKind; label: string; icon: typeof ScrollText }> = [
  { id: "script", label: "剧本", icon: ScrollText },
  { id: "shots", label: "镜头表", icon: Scissors },
  { id: "storyboard", label: "故事板", icon: Image },
  { id: "audio", label: "音频", icon: Music2 },
  { id: "video", label: "视频", icon: Film },
  { id: "compose", label: "合成", icon: Save },
];
...
function StoredMedia({ active, kind, storageKey, label }: { active: boolean; kind: "image" | "audio" | "video"; storageKey?: string; label: string }) {
  ...
  if (!storageKey) return <div className="grid min-h-32 place-items-center rounded-lg border border-dashed border-[var(--ob-line)] text-xs text-[var(--ob-muted)]">待生成</div>;
...
function ScriptView({ view }: { view: EpisodeProductionView }) {
  return <div className="space-y-4"><div><h3 className="font-medium">{view.episode.title}</h3><p className="mt-1 text-sm text-[var(--ob-muted)]">{view.episode.synopsis || "暂无分集梗概"}</p></div>{view.scenes.map((scene) => <article key={scene.id} className="rounded-xl border border-[var(--ob-line)] p-4"><h4 className="text-sm font-semibold">{scene.heading}</h4><p className="mt-1 text-sm text-[var(--ob-muted)]">{scene.synopsis}</p>{view.shots.filter((shot) => shot.sceneId === scene.id).map((shot) => <div key={shot.id} className="mt-3 border-l-2 border-[var(--ob-accent)] pl-3"><strong className="text-sm">{shot.title}</strong><p className="text-sm">{shot.description}</p>{view.dialogues.filter((dialogue) => dialogue.shotId === shot.id).map((dialogue) => <p key={dialogue.id} className="mt-2 text-sm"><span className="text-xs text-[var(--ob-muted)]">{dialogue.kind === "narration" ? "旁白" : "对白"}</span>　{dialogue.text}</p>)}</div>)}</article>)}</div>;
}
...
function ShotEditor({ shot, dialogues, identities, styles, busy, onSave, onCreateDialogue, onSaveDialogue, onDeleteDialogue }: { shot: FilmShot; dialogues: FilmDialogue[]; identities: FilmStatus["document"]["assets"]; styles: FilmStatus["document"]["assets"]; busy: boolean; onSave: (shot: FilmShot, patch: Partial<FilmShot>) => void; onCreateDialogue: (shotId: string, kind: FilmDialogue["kind"], text: string) => void; onSaveDialogue: (dialogue: FilmDialogue, patch: Partial<FilmDialogue>) => void; onDeleteDialogue: (dialogue: FilmDialogue) => void }) {
  ...
  return <article className="min-w-[42rem] border-b border-[var(--ob-line)] p-3 last:border-b-0" data-testid={`film-shot-${shot.id}`}><div className="grid gap-2 md:grid-cols-[8rem_minmax(14rem,1fr)_6rem_10rem_auto]"><strong className="self-center text-sm">{shot.title}</strong><input aria-label={`${shot.title} 描述`} className="ob-input" value={description} onChange={(event) => setDescription(event.target.value)} /><input aria-label={`${shot.title} 秒数`} className="ob-input" type="number" min="0.1" max="900" step="0.1" value={duration} onChange={(event) => setDuration(Number(event.target.value))} /><select aria-label={`${shot.title} 风格绑定`} className="ob-input" value={style} onChange={(event) => setStyle(event.target.value)}><option value="">无风格</option>{styles.map((asset) => <option key={asset.id} value={asset.id}>{asset.title}</option>)}</select><button type="button" className="ob-btn min-h-10" disabled={busy} onClick={() => onSave(shot, { description, durationSeconds: duration, styleAssetId: style, identityVersionIds: identityIds })}><Save aria-hidden="true" size={14} />保存镜头</button></div><fieldset className="mt-2 flex flex-wrap gap-3"><legend className="text-xs text-[var(--ob-muted)]">角色身份版本</legend>{identities.map((asset) => <label key={asset.id} className="text-xs"><input type="checkbox" checked={identityIds.includes(asset.id)} onChange={(event) => setIdentityIds((current) => event.target.checked ? [...current, asset.id] : current.filter((id) => id !== asset.id))} /> {asset.title}</label>)}</fieldset><div className="mt-3 space-y-2"><div className="flex flex-wrap items-center gap-1"><strong className="mr-auto text-xs">对白 / 旁白</strong><button type="button" className="ob-btn min-h-10" disabled={busy} onClick={() => onCreateDialogue(shot.id, "dialogue", "新对白")}>添加对白</button><button type="button" className="ob-btn min-h-10" disabled={busy} onClick={() => onCreateDialogue(shot.id, "narration", "新旁白")}>添加旁白</button></div>{dialogues.map((dialogue) => <DialogueEditor key={filmEditorKey(dialogue.id, dialogue.revision)} dialogue={dialogue} busy={busy} onSave={onSaveDialogue} onDelete={onDeleteDialogue} />)}</div></article>;
}
...
export function EpisodeProductionViews({ status, busy, initialView = "script", onSaveEpisode, onSaveShot, onCreateDialogue, onSaveDialogue, onDeleteDialogue }: {
  ...
  if (!document.episodes.length) return <WorkbenchSection id="episodes" title="分集制作视图 / Episode Production" wide><EmptyView>导入原稿并完成拆解后会出现分集制作视图。</EmptyView></WorkbenchSection>;
  ...
  const navigateTabs = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    const target = event.key === "Home" ? 0 : event.key === "End" ? views.length - 1 : event.key === "ArrowRight" ? (index + 1) % views.length : event.key === "ArrowLeft" ? (index - 1 + views.length) % views.length : -1;
    if (target >= 0) { event.preventDefault(); selectView(views[target]!.id); }
  };
  return <WorkbenchSection id="episodes" title="分集制作视图 / Episode Production" wide>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center"><label className="text-sm font-medium" htmlFor="film-episode-selector">当前分集</label><select id="film-episode-selector" className="ob-input min-w-0 flex-1 sm:max-w-sm" value={view.episode.id} onChange={(event) => setEpisodeId(event.target.value)}>{document.episodes.map((episode) => <option key={episode.id} value={episode.id}>{episode.order + 1}. {episode.title}</option>)}</select><div className="flex min-w-0 flex-1 gap-2"><input aria-label={`${view.episode.title} 集标题`} className="ob-input min-w-0 flex-1" defaultValue={view.episode.title} key={filmEditorKey(view.episode.id, view.episode.revision)} /><button type="button" className="ob-btn min-h-10 shrink-0" disabled={busy} onClick={(event) => onSaveEpisode(view.episode.id, view.episode.revision, (event.currentTarget.previousElementSibling as HTMLInputElement).value)}><Save aria-hidden="true" size={14} />保存集</button></div></div>
    <div role="tablist" aria-label={`${view.episode.title} 制作视图`} className="mt-4 flex gap-1 overflow-x-auto border-b border-[var(--ob-line)] pb-1">{views.map((item, index) => { const Icon = item.icon; const selected = activeView === item.id; return <button key={item.id} id={`episode-tab-${item.id}`} type="button" role="tab" aria-selected={selected} aria-controls={`episode-panel-${item.id}`} tabIndex={selected ? 0 : -1} className={`ob-tab min-h-10 shrink-0 focus-visible:outline-2 focus-visible:outline-offset-2 ${selected ? "text-[var(--ob-accent)]" : ""}`} onClick={() => setActiveView(item.id)} onKeyDown={(event) => navigateTabs(event, index)}><Icon aria-hidden="true" size={14} />{item.label}</button>; })}</div>
    ...
    <div id="episode-panel-shots" role="tabpanel" aria-labelledby="episode-tab-shots" tabIndex={activeView === "shots" ? 0 : -1} hidden={activeView !== "shots"} className="mt-4 focus-visible:outline-2 focus-visible:outline-offset-2"><div className="overflow-x-auto rounded-xl border border-[var(--ob-line)]">{view.shots.map((shot) => { const scene = view.scenes.find((item) => item.id === shot.sceneId)!; return <ShotEditor key={filmEditorKey(shot.id, shot.revision)} shot={shot} dialogues={view.dialogues.filter((dialogue) => dialogue.shotId === shot.id)} identities={identities.filter((identity) => (!identity.episodeIds?.length || identity.episodeIds.includes(view.episode.id)) && (!identity.sceneIds?.length || identity.sceneIds.includes(scene.id)) && (!identity.shotIds?.length || identity.shotIds.includes(shot.id)))} styles={styles} busy={busy} onSave={onSaveShot} onCreateDialogue={onCreateDialogue} onSaveDialogue={onSaveDialogue} onDeleteDialogue={onDeleteDialogue} />; })}{!view.shots.length ? <EmptyView>本集尚无镜头。</EmptyView> : null}</div></div>
    <div id="episode-panel-storyboard" role="tabpanel" aria-labelledby="episode-tab-storyboard" tabIndex={activeView === "storyboard" ? 0 : -1} hidden={activeView !== "storyboard"} className="mt-4 focus-visible:outline-2 focus-visible:outline-offset-2"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{view.shots.map((shot) => <article key={shot.id} className="rounded-xl border border-[var(--ob-line)] p-3"><StoredMedia active={activeView === "storyboard"} kind="image" storageKey={shot.imageStorageKey ?? shot.firstFrameStorageKey} label={`${shot.title} 故事板`} /><h3 className="mt-3 text-sm font-medium">{shot.title}</h3><p className="mt-1 text-sm text-[var(--ob-muted)]">{shot.description}</p></article>)}</div></div>
    <div id="episode-panel-audio" role="tabpanel" aria-labelledby="episode-tab-audio" tabIndex={activeView === "audio" ? 0 : -1} hidden={activeView !== "audio"} className="mt-4 focus-visible:outline-2 focus-visible:outline-offset-2"><div className="space-y-3">{view.shots.map((shot) => <article key={shot.id} className="rounded-xl border border-[var(--ob-line)] p-3"><h3 className="mb-2 text-sm font-medium">{shot.title} · 镜头音频</h3><StoredMedia active={activeView === "audio"} kind="audio" storageKey={shot.audioStorageKey} label={`${shot.title} 镜头音频`} />{view.dialogues.filter((dialogue) => dialogue.shotId === shot.id).map((dialogue) => <div key={dialogue.id} className="mt-3 border-t border-[var(--ob-line)] pt-3"><p className="mb-2 text-sm">{dialogue.text}</p><StoredMedia active={activeView === "audio"} kind="audio" storageKey={dialogue.audioStorageKey} label={`${shot.title} 对白音频`} /></div>)}</article>)}</div></div>
    <div id="episode-panel-video" role="tabpanel" aria-labelledby="episode-tab-video" tabIndex={activeView === "video" ? 0 : -1} hidden={activeView !== "video"} className="mt-4 focus-visible:outline-2 focus-visible:outline-offset-2"><div className="grid gap-3 sm:grid-cols-2">{view.shots.map((shot) => <article key={shot.id} className="rounded-xl border border-[var(--ob-line)] p-3"><StoredMedia active={activeView === "video"} kind="video" storageKey={shot.videoStorageKey} label={`${shot.title} 视频`} /><div className="mt-2 flex items-center gap-2 text-sm"><strong>{shot.title}</strong><span className="ml-auto text-[var(--ob-muted)]">{shot.durationSeconds.toFixed(1)} 秒</span></div></article>)}</div></div>
    <div id="episode-panel-compose" role="tabpanel" aria-labelledby="episode-tab-compose" tabIndex={activeView === "compose" ? 0 : -1} hidden={activeView !== "compose"} className="mt-4 focus-visible:outline-2 focus-visible:outline-offset-2"><div className="space-y-3">{view.timelineTracks.map((track) => <section key={track.id} className="rounded-xl border border-[var(--ob-line)] p-3"><div className="flex items-center gap-2"><h3 className="text-sm font-medium">{track.title}</h3><span className="ob-chip">{track.kind}</span><span className="ml-auto text-xs text-[var(--ob-muted)]">{track.clips.length} 片段</span></div><div className="mt-2 flex gap-2 overflow-x-auto">{track.clips.map((clip) => <div key={clip.id} className="min-w-40 rounded-lg bg-[var(--ob-canvas)] p-2 text-xs"><strong>{clip.id}</strong><p className="mt-1 break-all text-[var(--ob-muted)]">{clip.source}</p><p className="mt-1">{Math.max(0, clip.end - clip.start).toFixed(1)} 秒 · {clip.transition}</p></div>)}</div></section>)}{!view.timelineTracks.length ? <EmptyView>本集媒体尚未加入现有时间线。</EmptyView> : null}<a href="#timeline" className="ob-btn min-h-10">前往完整时间线编辑</a></div></div>
  </WorkbenchSection>;
}
