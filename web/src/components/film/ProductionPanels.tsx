...
export function ProductionPanel({ status, busy, onLegacyStage, onRun, onSynced, onWaive = () => {}, onRevokeWaiver = () => {} }: { status: FilmStatus; busy: boolean; onLegacyStage: (stage: FilmStage, action: "run" | "approve" | "reject") => void; onRun: (stage: FilmStageKind, request: FilmStageRunRequest) => Promise<boolean>; onSynced: (status: FilmStatus) => void; onWaive?: (stage: FilmStage, reason: string) => void; onRevokeWaiver?: (waiverId: string, waiverRevision: number) => void }) {
  ...
  const requestWaiver = (item: FilmStage) => {
    const reason = window.prompt("请输入豁免原因。该原因、操作者、风险确认和影响范围会写入审计记录。")?.trim();
    if (!reason || !window.confirm("确认接受跳过此阶段审批的制作风险？")) return;
    onWaive(item, reason);
  };
  return <WorkbenchSection id="tasks" title="阶段运行与 Generation Jobs" wide>
    <div className="grid gap-4 xl:grid-cols-[1fr_1.2fr]">
      <div><h3 className="text-sm font-medium">阶段审核状态</h3><ol className="mt-2 space-y-2">{status.document.stages.map((item) => { const waiver = activeWaivers.get(item.id); return <li key={item.id} data-testid={`film-stage-${item.id}`} className="flex flex-wrap items-center gap-2 rounded-lg border border-[var(--ob-line)] p-2"><span>{stageLabels[item.id]}</span><span className="mr-auto text-xs">{item.status} · r{item.revision}</span>{item.status === "needs_review" ? <><button className="ob-btn" onClick={() => onLegacyStage(item, "approve")}>批准</button><button className="ob-btn" onClick={() => onLegacyStage(item, "reject")}>退回</button></> : <button className="ob-btn" disabled={busy || item.status === "approved"} onClick={() => onLegacyStage(item, "run")}>提交产物审核</button>}{capabilities.features?.stageWaiver && item.status !== "approved" && !protectedStages.has(item.id) ? waiver ? <button className="ob-btn" disabled={busy} title={waiver.reason} onClick={() => onRevokeWaiver(waiver.id, waiver.revision)}>撤销豁免</button> : <button className="ob-btn" disabled={busy} onClick={() => requestWaiver(item)}>审计豁免</button> : null}{waiver ? <p className="w-full text-xs text-amber-500">已豁免：{waiver.reason}</p> : null}</li>; })}</ol><p className="mt-2 text-xs text-[var(--ob-muted)]">提交产物审核只改变审核状态，不代表媒体生成完成。豁免不会改写阶段状态，并会披露到质量报告与交付 manifest。</p></div>
