...
export function DeliveryPanel({ status, busy, onExport, onCancel, onRefresh }: { status: FilmStatus; busy: boolean; onExport: (kind: "mp4" | "srt" | "manifest" | "asset_bundle") => void; onCancel: (jobId: string) => void; onRefresh: () => void }) {
  ...
  return <WorkbenchSection id="delivery" title="异步交付 / Delivery">
    ...
    <ul className="mt-4 space-y-2">{status.document.deliverables.map((item) => <li key={item.id} className="rounded-lg border border-[var(--ob-line)] p-3"><div className="flex items-center gap-2"><strong className="mr-auto text-sm">{item.title}</strong><span className="text-xs">{item.kind} · {item.status}</span>{item.status === "running" && item.generationJobId ? <button className="ob-btn" disabled={busy} onClick={() => onCancel(item.generationJobId!)}>取消</button> : null}{item.status === "failed" || item.status === "canceled" ? <button className="ob-btn" disabled={busy} onClick={() => onExport(item.kind)}>重试</button> : null}</div>{item.diagnostic ? <p className="text-xs text-[var(--ob-danger)]">{item.diagnostic}</p> : null}{item.status === "approved" && (item.storageKey || item.content) ? <a className="ob-btn mt-2 inline-flex" href={filmDeliverableDownloadURL(status.document.projectId, item.id)}><Download size={14} /> 下载</a> : null}</li>)}</ul>
  </WorkbenchSection>;
}
