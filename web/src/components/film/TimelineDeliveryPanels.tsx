...
import { localizeFilmDiagnostic, localizeFilmKind, localizeFilmStatus, localizeFilmTransition } from "./film-display";
...
export function DeliveryPanel({ status, busy, onExport, onCancel, onRefresh }: { status: FilmStatus; busy: boolean; onExport: (kind: "mp4" | "srt" | "manifest" | "asset_bundle") => void; onCancel: (jobId: string) => void; onRefresh: () => void }) {
  ...
  return <WorkbenchSection id="delivery" title={t("film.delivery.title")}>
	    <div className="flex flex-wrap gap-2"><select aria-label={t("film.delivery.type")} className="ob-input" value={kind} onChange={(event) => setKind(event.target.value as typeof kind)}><option value="mp4" disabled={!status.capabilities.mp4Export}>{localizeFilmKind(t, "mp4")}</option><option value="srt">{localizeFilmKind(t, "srt")}</option><option value="manifest">{localizeFilmKind(t, "manifest")}</option><option value="asset_bundle" disabled={!status.capabilities.assetBundleExport}>{localizeFilmKind(t, "asset_bundle")}</option></select><button className="ob-btn ob-btn-primary" disabled={busy || disabled} onClick={() => onExport(kind)}>{t("film.delivery.request")}</button><button className="ob-btn" onClick={onRefresh}>{t("film.delivery.refresh")}</button></div>
	    {!status.capabilities.mp4Export ? <p className="mt-2 text-xs text-[var(--ob-danger)]">{t("film.delivery.mp4Unavailable", { diagnostic: localizeFilmDiagnostic(t, status.capabilities.mp4Diagnostic) })}</p> : null}{!status.capabilities.assetBundleExport ? <p className="mt-1 text-xs text-[var(--ob-muted)]">{t("film.delivery.bundleUnavailable")}</p> : null}
	    <ul className="mt-4 space-y-2">{status.document.deliverables.map((item) => <li key={item.id} className="rounded-lg border border-[var(--ob-line)] p-3"><div className="flex items-center gap-2"><strong className="mr-auto text-sm">{item.title}</strong><span className="text-xs">{localizeFilmKind(t, item.kind)} · {localizeFilmStatus(t, item.status)}</span>{item.status === "running" && item.generationJobId ? <button className="ob-btn" disabled={busy} onClick={() => onCancel(item.generationJobId!)}>{t("film.delivery.cancel")}</button> : null}{item.status === "failed" || item.status === "canceled" ? <button className="ob-btn" disabled={busy} onClick={() => onExport(item.kind)}>{t("film.delivery.retry")}</button> : null}</div>{item.diagnostic ? <p className="text-xs text-[var(--ob-danger)]">{localizeFilmDiagnostic(t, item.diagnostic)}</p> : null}{item.status === "approved" && (item.storageKey || item.content) ? <a className="ob-btn mt-2 inline-flex" href={filmDeliverableDownloadURL(status.document.projectId, item.id)}><Download size={14} /> {t("film.delivery.download")}</a> : null}</li>)}</ul>
  </WorkbenchSection>;
}
