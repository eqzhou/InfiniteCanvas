...
function StyleExtractionPanel({ status, channels, onFilmStatus }: PanelProps) {
  ...
  return <div className="space-y-3">
    ...
    <div><h4 className="text-xs font-medium uppercase text-[var(--ob-muted)]">{t("film.advanced.styleRuns")}</h4>{tasks.length ? tasks.map((task) => <p key={task.id} className="mt-1 text-xs">{task.title} · {task.status}{task.error ? ` · ${task.error}` : ""}</p>) : <p className="mt-1 text-xs text-[var(--ob-muted)]">{t("film.advanced.noRuns")}</p>}</div>
    <div><h4 className="text-xs font-medium uppercase text-[var(--ob-muted)]">{t("film.advanced.styleCandidates")}</h4>{candidates.length ? candidates.map((candidate) => <div key={candidate.id} className="mt-2 rounded border border-[var(--ob-line)] p-3 text-sm"><div className="flex flex-wrap items-center gap-2"><strong>{candidate.bible.summary}</strong><span className="text-xs text-[var(--ob-muted)]">{candidate.status} · {candidate.providerId}/{candidate.model}</span></div><p className="mt-1 text-xs">{candidate.bible.stylePrompt}</p>{candidate.status === "needs_review" ? <button type="button" className="ob-btn mt-2" disabled={busy} onClick={() => void run(() => adoptFilmStyleCandidate(status.document.projectId, candidate.id, { revision: status.document.revision, candidateRevision: candidate.revision, title: t("film.advanced.adoptedStyleTitle", { title: candidate.sourceAsset.title }) }), t("film.advanced.styleAdopted"))}><Check size={14} />{t("film.advanced.adoptCandidate")}</button> : null}</div>) : <p className="mt-1 text-xs text-[var(--ob-muted)]">{t("film.advanced.noCandidates")}</p>}</div>
  </div>;
}
...
function ComfyUIRunPanel({ status }: PanelProps) {
  ...
  return <div className="space-y-3">
    ...
    {job ? <p role="status" className="text-sm">{job.id} · {job.status} · {job.providerId}/{job.model}{job.error ? ` · ${job.error}` : ""}</p> : <p className="text-xs text-[var(--ob-muted)]">{t("film.advanced.noComfyJob")}</p>}{error ? <p role="alert" className="text-sm text-[var(--ob-danger)]">{error}</p> : null}
  </div>;
}
