  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2 md:grid-cols-3">
        <input className="ob-field" aria-label="搜索素材" placeholder="按标题或来源搜索" value={q} onChange={(event) => setQ(event.target.value)} />
        <select className="ob-field" aria-label="按类型筛选素材" value={kind} onChange={(event) => setKind(event.target.value as "all" | LibraryAssetKind)}>
          <option value="all">全部类型</option>
          {(Object.keys(KIND_LABELS) as LibraryAssetKind[]).map((value) => (
            <option key={value} value={value}>{KIND_LABELS[value]}</option>
          ))}
        </select>
        <input className="ob-field" aria-label="按标签筛选素材" placeholder="标签筛选" value={tag} onChange={(event) => setTag(event.target.value)} />
      </div>
      ...
      <section className="space-y-2 rounded-xl border border-[var(--ob-line)] p-3">
        ...
        <div className="flex gap-2">
          <button type="button" className="ob-btn is-primary" disabled={!draft.title.trim()} onClick={save}>
            {editing ? "保存素材" : "新增素材"}
          </button>
          {editing ? (
            <button type="button" className="ob-btn" onClick={() => { setDraft(EMPTY_DRAFT); setEditing(false); }}>取消编辑</button>
          ) : null}
        </div>
      </section>
      ...
      <div className="space-y-1">
        {items.map((item) => (
          <div key={item.id} className="flex items-start gap-2 rounded-lg border border-[var(--ob-line)] p-2">
            ...
            <button type="button" className="ob-btn text-[var(--ob-danger)]" aria-label={`删除素材 ${item.title}`} onClick={() => {
