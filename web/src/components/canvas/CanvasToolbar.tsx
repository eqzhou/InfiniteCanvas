...
export function CanvasToolbar({
  ...
  return (
    <div
      ...
      aria-label="画布工具栏"
      className={cn(
        "ob-toolbar-scroll z-30 flex min-h-14 w-full min-w-0 max-w-full flex-nowrap items-center gap-1.5 overflow-x-auto border-b border-[var(--ob-line)] bg-[var(--ob-panel-glass)] py-2 pr-3 shadow-[var(--ob-shadow)] backdrop-blur-md",
    ...
    >
      ...
      <label aria-label="导入图片" title="导入图片" className="ob-btn shrink-0 cursor-pointer px-2.5 py-1.5">
        <Upload size={16} />
      ...
      {onImportVideos ? (
        <label aria-label="导入视频" title="导入视频" className="ob-btn shrink-0 cursor-pointer px-2.5 py-1.5">
          <Film size={16} />
      ...
      {onImportAudios ? (
        <label aria-label="导入音频" title="导入音频" className="ob-btn shrink-0 cursor-pointer px-2.5 py-1.5">
          <Music2 size={16} />
...
function Tool({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className={`inline-flex min-h-9 shrink-0 items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-sm font-medium transition-colors ${
        active
