...
export function TaskCenterTable({ items, onCancel }: { items: TaskCenterItem[]; onCancel: (id: string) => void }) {
  return <div className="overflow-x-auto rounded-xl border border-[var(--ob-line)]"><table className="w-full min-w-[940px] text-left text-sm"><thead className="bg-[var(--ob-surface-2)] text-xs text-[var(--ob-muted)]"><tr><th className="p-3">任务</th><th className="p-3">来源</th><th className="p-3">状态</th><th className="p-3">进度</th><th className="p-3">算力</th><th className="p-3">更新时间</th><th className="p-3">操作</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t border-[var(--ob-line)]"><td className={`p-3 ${item.parentTaskId ? "pl-8" : ""}`}><strong>{item.parentTaskId ? "↳ " : ""}{item.title}</strong><div className="mt-1 max-w-md truncate text-xs text-[var(--ob-muted)]">{item.id}{item.error ? ` · ${item.error}` : ""}</div></td><td className="p-3">{item.source === "film" ? `影视制片${item.stage ? ` · ${item.stage}` : ""}` : item.source}</td><td className="p-3">{statusLabels[item.status]}</td><td className="p-3 text-xs">{item.progress === undefined ? "—" : <div className="min-w-28"><div>{Math.round(item.progress * 100)}%{item.total !== undefined ? ` · ${item.succeeded ?? 0}/${item.total}` : ""}{item.failed ? ` · 失败 ${item.failed}` : ""}</div><progress className="mt-1 w-full" max={1} value={item.progress} aria-label={`${item.title}进度`} /></div>}</td><td className="p-3 text-xs">{item.estimatedCredits === undefined ? "—" : `实际 ${item.actualCredits ?? 0} / 预计 ${item.estimatedCredits}`}</td><td className="p-3 text-xs">{new Date(item.updatedAt).toLocaleString()}</td><td className="p-3"><div className="flex gap-2"><Link className="ob-btn" to={item.sourcePath}>打开来源</Link>{item.status === "queued" || item.status === "running" ? <button type="button" className="ob-btn" onClick={() => onCancel(item.id)}><XCircle size={14} />取消</button> : null}</div></td></tr>)}</tbody></table>{!items.length ? <p className="p-6 text-center text-sm text-[var(--ob-muted)]">没有符合条件的持久任务。</p> : null}</div>;
}
