...
func Mount(r chi.Router, dataDir string) {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Post("/generation-jobs/{id}/recover", s.recoverGenerationJob)
		...
		r.Get("/tenant/policy", s.getTenantPolicy)
		r.Put("/tenant/policy", s.putTenantPolicy)
		r.Get("/platform/policy", s.getPlatformPolicy)
		r.Put("/platform/policy", s.putPlatformPolicy)
		...
		r.Get("/media-capabilities", s.getMediaCapabilities)
		r.Put("/admin/channels", s.putAdminChannels)
		r.Post("/admin/channels/preview-models", s.previewAdminChannelModels)
		r.Post("/admin/channels/preview-test", s.previewAdminChannelTest)
		r.Put("/admin/channels/{id}", s.putAdminChannel)
		...
		r.Get("/admin/tenant-quota", s.getAdminTenantQuota)
		r.Put("/admin/tenant-quota", s.putAdminTenantQuota)
		...
		// Explicit tenant control-plane routes. The /admin aliases above remain
		// temporarily for older clients, but all new UI traffic uses /tenant.
		r.Get("/tenant/quota", s.getAdminTenantQuota)
		r.Get("/tenant/members", s.listAdminUsers)
		r.Patch("/tenant/members/{id}", s.patchAdminUser)
		r.Get("/tenant/credit-logs", s.listAdminCreditLogs)
		r.Get("/tenant/channels", s.getAdminChannels)
		r.Put("/tenant/channels", s.putAdminChannels)
		r.Post("/tenant/channels/preview-models", s.previewAdminChannelModels)
		r.Post("/tenant/channels/preview-test", s.previewAdminChannelTest)
		r.Put("/tenant/channels/{id}", s.putAdminChannel)
		r.Delete("/tenant/channels/{id}", s.deleteAdminChannel)
		r.Put("/tenant/channels/{id}/secret", s.putAdminChannelSecret)
		r.Post("/tenant/channels/{id}/test", s.testAdminChannel)
		r.Post("/tenant/channels/{id}/models", s.getAdminChannelModels)
		r.Post("/tenant/invitations", s.createTenantInvitation)
		r.Get("/tenant/invitations", s.listTenantInvitations)
		r.Post("/tenant/invitations/{id}/revoke", s.revokeTenantInvitation)
		r.Get("/platform/tenants", s.listPlatformTenants)
		r.Get("/platform/users", s.listPlatformUsers)
		r.Get("/platform/channels", s.getPlatformChannels)
		r.Put("/platform/channels", s.putPlatformChannels)
		r.Post("/platform/channels/preview-models", s.previewPlatformChannelModels)
		r.Post("/platform/channels/preview-test", s.previewPlatformChannelTest)
		r.Put("/platform/channels/{id}", s.putPlatformChannel)
		r.Delete("/platform/channels/{id}", s.deletePlatformChannel)
		r.Put("/platform/channels/{id}/secret", s.putPlatformChannelSecret)
		r.Post("/platform/channels/{id}/test", s.testPlatformChannel)
		r.Post("/platform/channels/{id}/models", s.getPlatformChannelModels)
		r.Post("/platform/channels/migrate-local", s.migrateLocalChannelsToPlatform)
		r.Get("/platform/model-costs", s.getPlatformModelCosts)
		r.Put("/platform/model-costs", s.putAdminModels)
		r.Put("/platform/tenants/{tenantId}/quota", s.updatePlatformTenantQuota)
		r.Patch("/platform/users/{userId}", s.patchPlatformUser)
		r.Put("/platform/users/{userId}/password", s.resetPlatformUserPassword)
		r.Post("/platform/users/{userId}/credit-adjustments", s.adjustPlatformUserCredits)
		...
		r.Get("/tenant/storage-pool", s.getAdminStoragePool)
		r.Put("/tenant/storage-pool", s.putAdminStoragePool)
		r.Put("/tenant/storage-pool/{id}/secret", s.putAdminStoragePoolSecret)
		r.Delete("/tenant/storage-pool/{id}", s.deleteAdminStoragePoolProvider)
		...
		r.Get("/tenant/prompt-catalog", s.getAdminPromptCatalog)
		r.Post("/tenant/prompt-categories", s.createAdminPromptCategory)
		r.Put("/tenant/prompt-categories/{id}", s.putAdminPromptCategory)
		r.Delete("/tenant/prompt-categories/{id}", s.deleteAdminPromptCategory)
		r.Post("/tenant/prompts", s.createAdminPrompt)
		r.Put("/tenant/prompts/{id}", s.putAdminPrompt)
		r.Post("/tenant/prompts/bulk-delete", s.bulkDeleteAdminPrompts)
		r.Post("/tenant/prompt-sources", s.createAdminPromptSource)
		r.Put("/tenant/prompt-sources/{id}", s.putAdminPromptSource)
		r.Delete("/tenant/prompt-sources/{id}", s.deleteAdminPromptSource)
		r.Post("/tenant/prompt-sources/{id}/sync", s.syncAdminPromptSource)
		r.Post("/tenant/prompt-sources/sync-all", s.syncAllAdminPromptSources)
		r.Post("/tenant/prompt-sources/run-due", s.runDueAdminPromptSources)
...
func (s *Server) agentStatus(w http.ResponseWriter, r *http.Request) {
	hostExecution := accountAgentExecutionAllowedForRequest(r)
...
func (s *Server) deleteProject(w http.ResponseWriter, r *http.Request) {
	if s.store != nil && !s.requireTenantOwner(w, r, "tenant projects unavailable") {
		return
	}
	release, err := s.acquireWriteLock()
	if err != nil {
