...
### 2.2.2 复审补充（2026-07-26 三次审计新增）

对 Phase A–D 五个提交做代码审查时，发现 T2-03 的实现只完成了一半：白名单被持久化、写入时自校验、前台选择器据其收窄，但**服务端生成路径从不读取它**。`loadSitePolicy` 的调用点只有注册与云端渠道开关两处，四个生成入口（image/video/audio/workflow）均未校验 `model`。这意味着白名单只是前端的显示过滤，直接 POST 任务即可使用任意模型——治理规则形同虚设。

| ID | 差异 | 上游出处 | 状态 | 说明 |
|---|---|---|---|---|
| T2-24 | 模型白名单未在服务端生成路径强制执行 | features「AI 生成」、system-settings（`availableModels` 作为租户治理项） | ✅ 已修复 | 新增 `modelAllowedByPolicy` / `requireAllowedModel`，在 `createServerImageJob`、`createServerVideoJob`、`createServerAudioJob` 与 `createServerWorkflowJob` 四个入口校验。工作流的每个步骤自带 `model`，是第二条绕过路径，同样逐步校验。空白名单仍表示「不限制」，空 model 表示走渠道默认值（管理员配置的值，不属于用户选择），二者均放行；越界请求返回 403 并说明原因 |
| T2-25 | 生成请求失败时前端丢弃服务端给出的原因 | —（本地缺陷，非上游差异） | ✅ 已修复 | `generation-jobs.ts` 的 `api()` 把任何非 2xx 一律折叠成 `HTTP <status>`，T2-24 新增的 403 与既有的「云端渠道已被管理员关闭」都会退化为无信息的状态码。改为保留服务端文本（超长或 HTML 错误页则回落到状态码），使拒绝原因可被用户看到 |
| T2-26 | 默认模型与失效兜底从未在生产路径生效 | system-settings（T2-05 的实际落地） | ✅ 已修复 | `resolveDefaultModel` 写好并有单测，但全仓仅测试文件引用，生产代码零调用——T2-05 实为死代码。新增 `reconcileProviderModel` 并接入设置面板的模型拉取流程：拉到渠道模型后，未设置的模型按租户默认播种，渠道已下线的模型被替换，用户显式且仍可选的选择不被覆盖；列表为空（尚未拉取）时保持原值，避免清掉手填模型 |
...
- [x] 在服务端四个生成入口强制执行白名单（三次审计补齐，见 T2-24）。前台收窄只是可用性，治理边界必须在服务端成立。
...
验收：管理员设置白名单后，前台模型选择被收窄，且**直接调用生成接口也会被服务端以 403 拒绝**；白名单为空时仍可用已启用渠道模型；音频 speed/instructions 到达 provider 请求体；全局默认值被新节点继承。
...
- Web：518 项 Bun 单元/集成测试通过；根目录 typecheck 与 production build 通过。
- Go：`go test ./...`、`go vet ./...` 通过。
- 浏览器：production Chromium 101/101 通过，含四角缩放（逐角拖拽 + 锚点断言）与节点信息两个新场景。
- Formal：本机 PostgreSQL/Redis 隔离 run 7/7 通过。
- 供应链：clean-room 标识扫描与 OSV 226 包审计通过。

## 6. 审查轮次记录

计划落地后又做了两轮代码审查，均产出真实缺陷，一并记录以免后续重复排查：

- **二次审计**：发现 T2-23（云端图片编辑对不支持的协议仍提供入口）。
- **三次审计**（对 Phase A–D 五个提交逐文件复核）：发现 T2-24（白名单未在服务端强制执行）、T2-25（前端丢弃服务端拒绝原因），以及若干实现缺陷——文本模型兜底会误选 TTS 模型、锁定比例在触及最小尺寸时被破坏、参考媒体公开令牌对所有协议无条件铸造且 TTL 用满 24 小时、过期令牌从不清理、`putSitePolicy` 缺 EOF 校验、后台素材库存在请求竞态、批量删除会删掉被筛选隐藏的条目。

两轮审计的共同教训：**「已实现」的判定必须以运行路径为准，而非以代码存在为准**。T2-05 的 `resolveDefaultModel` 与 T2-03 的白名单都写好了函数并有单测，但前者从未被生产代码调用、后者从未被服务端读取，仅凭符号存在与测试通过会得出错误结论。
