...
| T2-27 | 全局生成默认值无编辑入口 | features「可配置项」（T2-06/T2-07 的实际落地） | ✅ 已修复 | `generationDefaults` 有类型、归一化、持久化与消费方（新节点继承、音频参数解析），唯独没有任何写入点：设置面板零引用，用户只能用内置默认值。新增「生成默认值」编辑区（视频比例/清晰度/时长/生成声音/水印、音频格式/声音/语速/指令），E2E 覆盖「改值 → 新节点继承 → 重载后仍在」 |
...
- 浏览器：production Chromium 102/102 通过，含四角缩放（逐角拖拽 + 锚点断言）、节点信息、全局生成默认值三个新场景。
...
- **完成审计**（逐条验证「已实现」条目的运行路径，而非仅确认符号存在）：又发现两条只完成一半的条目——T2-26（`resolveDefaultModel` 零生产调用）、T2-27（`generationDefaults` 零写入点）。核验方法是对每个条目的关键符号统计**排除测试文件后**的引用数，引用数为 0 即为死代码，为 1 时继续追调用链确认不是被另一个死模块引用。
